...
**并行和异步编程几乎无痛点。**Go 语言的 Goroutine 和 Channel 这两个神器简直就是并发和异步编程的巨大福音。像 C、C++、Java、Python 和 JavaScript 这些语言的并发和异步方式太控制就比较复杂了，而且容易出错，而 Go 解决这个问题非常地优雅和流畅。这对于编程多年受尽并发和异步折磨的编程者来说，完全就是让人眼前一亮的感觉。Go 是一种非常高效的语言，高度支持并发性。Go是为大数据、微服务、并发而生的一种编程语言。Go 是为大数据、微服务、并发而生的一种语言。因为他有Goroutine and Channel 
...
C,Pascal and CSP , go 的语法、数据类型、控制流等都继承C语言，Go的包、面对思想源于Pascal 分支，and go 的管道通信协程并发，则是借助CSP分支。
...
go 语法和Erlang 、Python 类似，所以我们将这个三门语言做详细对比。
...
### Go 语言拥有协程，垃圾空间回收、栈空间管理，包管理系统，一等公民函数等

### go 为静态语言。 保证了再运行效率、内存用量、类型安全都比Python 和Erlang语言要强的多。

#### 对比Python和 Erlang 语言 Go语言除了支持表和字典等复杂数据结构以外还支持了，指针和接口类型。特别是接口类型功能特别强大，它提供管理类型系统的手段。而 指针提供了内存管理手段。
